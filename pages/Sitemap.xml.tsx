import { getAllProducts, getClient } from 'lib/sanity.client'
...
export async function getServerSideProps({ res }) {
  ...
  const [products = []] = await Promise.all([getAllProducts(client)])
  const productUrls: SitemapLocation[] = products
    .filter(({ slug = '' }) => slug)
    .map((product) => {
      return {
