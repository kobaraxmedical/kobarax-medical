/**
 * This code is responsible for revalidating the cache when a product or author is updated.
 *
 ...
 * 6. Filter: _type == "product" || _type == "author" || _type == "settings"
...
type StaleRoute = '/' | `/product-single/${string}`
...
async function queryStaleRoutes(
  ...
  if (body._type === 'product') {
    ...
    if (!exists) {
      ...
      if ((body.slug as any)?.current) {
        staleRoutes.push(`/product-single/${(body.slug as any).current}`)
      }
      // Assume that the product document was deleted. Query the datetime used to sort "More stories" to determine if the product was in the list.
      const moreStories = await client.fetch(
        groq`count(
          *[_type == "product"] | order(date desc, proImg _updatedAt desc) [0...3] [dateTime(date) > dateTime($date)]
        )`,
        { date: body.date },
      )
      // If there's less than 3 products with a newer date, we need to revalidate everything
      if (moreStories < 3) {
  ...
  switch (body._type) {
    ...
    case 'product':
      return await queryStaleProductRoutes(client, body._id)
    case 'settings':
...
async function _queryAllRoutes(client: SanityClient): Promise<string[]> {
  return await client.fetch(groq`*[_type == "product"].slug.current`)
}

async function queryAllRoutes(client: SanityClient): Promise<StaleRoute[]> {
  const slugs = await _queryAllRoutes(client)

  return ['/', ...slugs.map((slug) => `/product-single/${slug}` as StaleRoute)]
}
...
async function mergeWithMoreStories(
  ...
  const moreStories = await client.fetch(
    groq`*[_type == "product"] | order(date desc, _updatedAt desc) [0...3].slug.current`,
  )
...
async function queryStaleAuthorRoutes(
  ...
  let slugs = await client.fetch(
    ...
    "slug": *[_type == "product" && references(^._id)].slug.current
  }["slug"][]`,
    { id },
  )
  ...
  if (slugs.length > 0) {
    ...
    return ['/', ...slugs.map((slug) => `/product-signle/${slug}`)]
  }

  return []
}

async function queryStaleProductRoutes(
  client: SanityClient,
  id: string,
): Promise<StaleRoute[]> {
  let slugs = await client.fetch(
    groq`*[_type == "product" && _id == $id].slug.current`,
    { id },
  )

  slugs = await mergeWithMoreStories(client, slugs)

  return ['/', ...slugs.map((slug) => `/product-single/${slug}`)]
}
